const DataImporter = ({ onImport }) => {
  ...
  return (
    <div className="bg-gray-800 p-4 rounded-xl border border-gray-700 mt-4">
      <h3 className="font-bold text-gray-300 mb-2 flex items-center gap-2"><Upload size={16} /> 生徒データ一括登録 (CSV)</h3>
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
          ...
          <div className="flex items-center gap-2 bg-gray-800 p-2 rounded-lg border border-gray-700">
            <button onClick={() => setIsDemoMode(!isDemoMode)} className="flex items-center gap-2 text-xs font-bold px-2">{isDemoMode ? <ToggleRight size={24} className="text-green-400" /> : <ToggleLeft size={24} className="text-gray-500" />} Demo Mode</button>
            {isDemoMode && phase === window.PHASE.INPUT && (<button onClick={onGenerateDemo} className="bg-gray-700 hover:bg-gray-600 text-xs px-3 py-1 rounded border border-gray-600 flex items-center gap-1"><Database size={12} /> +50 Mock</button>)}
          </div>
          <div className="flex gap-2">
            {phase === window.PHASE.INPUT && (<button onClick={() => updatePhase(window.PHASE.CHECK)} className="bg-gradient-to-r from-green-500 to-emerald-600 px-6 py-3 rounded-xl font-bold shadow-lg hover:scale-105 transition flex items-center gap-2">入力締切 → チェックへ <ArrowRight size={18} /></button>)}
            {phase === window.PHASE.CHECK && (<><button onClick={() => updatePhase(window.PHASE.INPUT)} className="bg-gray-700 hover:bg-gray-600 px-4 py-3 rounded-xl font-bold transition flex items-center gap-2"><ArrowLeft size={18} /> 戻る</button><button onClick={handleMatching} className="bg-gradient-to-r from-purple-500 to-indigo-600 px-6 py-3 rounded-xl font-bold shadow-lg hover:scale-105 transition flex items-center gap-2"><Layers size={18} /> マッチング実行</button></>)}
            {phase === window.PHASE.MATCHING && (<><button onClick={() => updatePhase(window.PHASE.CHECK)} className="bg-gray-700 hover:bg-gray-600 px-4 py-3 rounded-xl font-bold transition flex items-center gap-2"><ArrowLeft size={18} /> 戻る</button><button onClick={handleMatching} className="bg-gray-700 hover:bg-gray-600 px-4 py-3 rounded-xl font-bold transition flex items-center gap-2"><RefreshCw size={18} /> 再生成</button><button onClick={handlePublish} className="bg-gradient-to-r from-orange-500 to-red-500 px-6 py-3 rounded-xl font-bold shadow-lg hover:scale-105 transition flex items-center gap-2"><Star size={18} fill="white" /> 公開する</button></>)}
            {phase === window.PHASE.PUBLISH && (<><button onClick={() => updatePhase(window.PHASE.MATCHING)} className="bg-gray-700 hover:bg-gray-600 px-4 py-3 rounded-xl font-bold transition flex items-center gap-2"><ArrowLeft size={18} /> 戻る</button><button onClick={onExport} className="bg-blue-600 hover:bg-blue-500 px-6 py-3 rounded-xl font-bold shadow-lg transition flex items-center gap-2"><Download size={18} /> CSV出力</button><div className="flex flex-col gap-1"><button onClick={reset} className="border border-red-500/50 text-red-400 hover:bg-red-500/10 px-4 py-2 rounded-lg font-bold text-xs transition">リセット</button><button onClick={onDeleteAll} className="bg-red-900/30 text-red-500 hover:bg-red-900/50 px-4 py-1 rounded-lg font-bold text-[10px] transition border border-red-900">全データ削除</button></div></>)}
          </div>
        </div>
        {phase === window.PHASE.INPUT && <DataImporter onImport={onImportData} />}
